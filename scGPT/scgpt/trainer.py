...
def train(
    ...
    for batch, batch_data in enumerate(loader):
        ...
        if config.task == "multiomic" and config.use_mod:
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            ...
            # Only add mod_types if use_mod is True and task is multiomic
            if config.use_mod and config.task == "multiomic":
...
def define_wandb_metrcis():
    ...
    # I added these metrics to track the training progress
    # Add more metrics to track
    wandb.define_metric("train/gep", summary="min", step_metric="epoch")
    wandb.define_metric("train/mvc", summary="min", step_metric="epoch")
    wandb.define_metric("train/cls", summary="min", step_metric="epoch")
    wandb.define_metric("train/ecs", summary="min", step_metric="epoch")
    wandb.define_metric("train/dab", summary="min", step_metric="epoch")
    ...
    wandb.define_metric("test/macro_f1", summary="max")
    wandb.define_metric("test/micro_f1", summary="max")
    # end of the added metrics
...
def evaluate(
    ...
    # Initialize lists to store predictions and labels for accuracy metrics
    all_predictions = []
    all_labels = []
        ...
        for batch_data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=config.amp):
                ...
                if config.task == "annotation":
                    ...
                    # Store predictions for accuracy metrics
                    predictions = output_values.argmax(1).cpu().numpy()
    ...
    # Add accuracy metrics if this is an annotation task
    if config.task == "annotation" and len(all_predictions) > 0:
        metrics_to_log.update({
            "valid/accuracy": accuracy_score(all_labels, all_predictions),
            "valid/precision": precision_score(all_labels, all_predictions, average="macro"),
            "valid/recall": recall_score(all_labels, all_predictions, average="macro"),
            "valid/macro_f1": f1_score(all_labels, all_predictions, average="macro"),
            "valid/micro_f1": f1_score(all_labels, all_predictions, average="micro"),
        })
