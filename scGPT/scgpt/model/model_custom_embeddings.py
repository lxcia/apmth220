...
class TransformerModel(nn.Module):
    def __init__(
        ...
        # Initialize encoder with custom embeddings if provided
        self.encoder = GeneEncoder(ntoken, d_model, padding_idx=vocab[pad_token])
        if custom_embeddings is not None:
            assert custom_embeddings.shape[0] == ntoken, \
                f"Custom embeddings shape {custom_embeddings.shape} does not match ntoken {ntoken}"
            assert custom_embeddings.shape[1] == d_model, \
                f"Custom embeddings dimension {custom_embeddings.shape[1]} does not match d_model {d_model}"
            self.encoder.embedding.weight.data = custom_embeddings
            # Keep embeddings learnable by not setting requires_grad to False
        ...
        # Initialize weights (except for custom embeddings which are already set)
        self.init_weights()

    def init_weights(self) -> None:
        # Only initialize weights that haven't been set by custom embeddings
        if not hasattr(self.encoder.embedding.weight, 'data') or self.encoder.embedding.weight.data is None:
            initrange = 0.1
