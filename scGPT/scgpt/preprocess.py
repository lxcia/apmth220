...
class Preprocessor:
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        """
        Preprocess the data and return a dictionary containing the processed data.
        """
        ...
        result_dict = {
            'adata': adata,  # Store the processed AnnData object
            'raw_counts': adata.raw.X if adata.raw is not None else adata.X.copy(),  # Store raw counts
            'normalized_counts': adata.X.copy(),  # Store normalized counts
            'var_names': adata.var_names.tolist(),  # Store gene names
            'obs_names': adata.obs_names.tolist(),  # Store cell names
        }

        # Add binning information if applicable
        if self.binning:
            logger.info("Binning data ...")
            if not isinstance(self.binning, int):
                raise ValueError(
            ...
            result_dict['binned_data'] = np.stack(binned_rows)
            result_dict['bin_edges'] = np.stack(bin_edges)

        # Add additional metadata
        result_dict['metadata'] = {
            'n_cells': adata.n_obs,
            'n_genes': adata.n_vars,
            'hvg_genes': adata.var_names[adata.var['highly_variable']].tolist() if 'highly_variable' in adata.var else None,
            'preprocessing_params': {
                'target_sum': self.normalize_total,
                'n_hvg': self.subset_hvg,
                'log1p': self.log1p,
