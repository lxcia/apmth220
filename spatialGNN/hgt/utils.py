...
def check_data_leakage(train_graph, val_graph, test_graph):
    """
    ...
    def get_edge_set(graph):
        ...
        for etype in graph.canonical_etypes:
            ...
            edge_tuples = list(zip(edges[0].tolist(), edges[1].tolist()))
            edge_set.update((e[0], e[1], etype) for e in edge_tuples)
        return edge_set

    train_edges = get_edge_set(train_graph)
    val_edges = get_edge_set(val_graph)
    test_edges = get_edge_set(test_graph)

    # Check overlap
    train_val_overlap = len(train_edges.intersection(val_edges))
    train_test_overlap = len(train_edges.intersection(test_edges))
    val_test_overlap = len(val_edges.intersection(test_edges))
