...
def read_cxg_data(f):
    ts_data = sc.read_h5ad(f)
    print(ts_data)
    if ("tissue" in ts_data.obs.columns):
        ts_data.obs["orig_tissue"] = ts_data.obs["tissue"] 
        ts_data.obs["orig_cell_type"] = ts_data.obs["cell_type"]
    ts_data.obs["orig_tissue"] = ts_data.obs["orig_tissue"].str.lower() 
    ts_data.obs["orig_cell_type"] = ts_data.obs["orig_cell_type"].str.lower()    
    ts_data.obs["organ_tissue"] = ts_data.obs["orig_tissue"]
    ts_data.obs["cell_ontology_class"] = ts_data.obs["orig_cell_type"]
    print(ts_data.obs[["organ_tissue", "cell_ontology_class"]])
    print(ts_data.obs["cell_ontology_class"].unique())

    cutoff = 5
    x = pd.DataFrame(np.unique(ts_data.obs["orig_cell_type"], return_counts=True)).T
    low_counts = list(set(x[x[1]<cutoff][0]))
    ts_data = ts_data[~np.isin(ts_data.obs["orig_cell_type"], low_counts)]
