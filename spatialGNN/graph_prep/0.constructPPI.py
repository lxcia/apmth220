...
from utils import load_global_PPI, read_ts_data, read_cxg_data, count_cells_per_celltype
from utils import calculate_correlation


TABULA_SAPIENS_DIR = "/n/data1/hms/dbmi/zitnik/lab/users/kel331/virtualcellsim/XL-PINNACLE/data/preprocessed/downsampled2_adata_normalized_log.h5ad"
PPI_DIR = "../data/networks/global_ppi_edgelist.txt"
OUTPUT_DIR = "../data/networks/"
adata = sc.read_h5ad(TABULA_SAPIENS_DIR, backed="r")
...
def extract_celltype_ppi(input_f, output_f, ppi, lcc = True, max_pval = 1, max_number_of_genes = 4000):
    ...
    for i, c in enumerate(sorted(celltype_ppi)):
        ...
        if "ENS" in c_df_sig.iloc[0, 0]:
            print("Converting Ensemble IDs to Gene IDs")
            c_df_sig.columns = ["Ensemble IDs", "p_value"]
            joined = pd.merge(c_df_sig, adata.var, left_on="Ensemble IDs", right_index=True, how="left")
            c_df_sig_genes = joined["names"].tolist()
        else:   
            c_df_sig_genes = c_df_sig[c + "_n"].tolist()
        
        c_subgraph = ppi.subgraph(c_df_sig_genes)
        print(len(c_subgraph.nodes))
        print(nx.connected_components(c_subgraph))
        if lcc:
            try:
                c_lcc = max(nx.connected_components(c_subgraph), key=len)
            except Exception as e:
                print(e)
                print("skipping cell type")
                continue
...
def write_ppi_edgelists(ppi_layers, output_f):
    ...
    for celltype, ppi in ppi_layers.items():
        output_edgelist_f = output_f + "_".join(celltype.lower().replace("/","\\").split(" ")) + ".txt"
...
def main():
    ...
    if args.rank:
        ...
        ts_data = read_cxg_data(TABULA_SAPIENS_DIR)
    ...
    # Extract cell type specific PPI
        ...
        write_ppi_edgelists(ppi_layers, OUTPUT_DIR + "ppi_edgelists_cellxgene/")
