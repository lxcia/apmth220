...
def EM_algorithm(X, K, 
    ...
    X = X.to('cuda:0')
    alpha0_hat = torch.tensor(alpha0_hat).to('cuda:0')
    m0_hat = torch.tensor(m0_hat).to('cuda:0')
    kappa0_hat = torch.tensor(kappa0_hat).to('cuda:0')
    S0_hat = torch.tensor(S0_hat).to('cuda:0')
    rho0_hat = torch.tensor(rho0_hat).to('cuda:0')
    clusters = torch.tensor(clusters).to('cuda:0')
